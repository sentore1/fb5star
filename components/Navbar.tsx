...
export default function Navbar({ 
  ...
  onCategoryChange,
  categories = ['All', 'Laptops', 'Desktops', 'Monitors', 'Accessories'],
  siteLogo,
  siteName = 'Fb5Star',
  headerStyle = 'minimal',
  logoSize = 40
  ...
  const getHeaderClasses = () => {
    const borderClass = isHomePage ? 'border-b-0' : 'border-b border-gray-100'
    
    switch(headerStyle) {
      case 'fashion':
        return 'bg-black text-white border-b-0'
      case 'classic':
        return `bg-white ${isHomePage ? 'border-b-0' : 'border-b-2 border-gray-900'}`
      case 'modern':
        return `bg-gradient-to-r from-gray-50 to-white ${isHomePage ? 'border-b-0' : 'border-b border-gray-200'}`
      default:
        return `bg-white ${borderClass}`
    }
  }
        ...
        <div className="flex justify-between items-center" style={{ minHeight: `${Math.max(64, logoSize + 16)}px` }}>
          <Link href="/" className="flex items-center space-x-1">
            {siteLogo ? (
              ...
              <span className={`text-2xl font-bold tracking-wide font-playfair ${headerStyle === 'fashion' ? 'text-white' : 'text-black'}`} style={{ fontFamily: 'var(--font-playfair)' }}>
                {siteName}
              </span>
            )}
          </Link>
            ...
            {isHomePage && (
              <div className="hidden md:block relative">
                ...
                <input
                  ...
                  className="pl-10 pr-4 py-1.5 w-64 border border-gray-200 rounded-full text-sm focus:outline-none focus:ring-1 focus:ring-black focus:border-transparent"
                />
              </div>
            )}
